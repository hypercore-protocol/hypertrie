...
Get.prototype._sendExt = function (seq) {
  if (this._extension && !this._db.feed.bitfield.get(seq)) {
...
Get.prototype._updateHeadCollides = function (i, bucket, val) {
  ...
  for (var j = val; j < bucket.length; j += 5) {
    ...
    if (!this._extensionSent) this._sendExt(seq)

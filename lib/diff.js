...
Diff.prototype._finalize = function () {
  ...
  while (this._checks.length) {
    this._maybeCollides(this._checks.pop(), this._checks.pop())
  }
...
Diff.prototype._next = function (cb) {
  ...
  while (this._stack.length) {
    ...
    for (var j = 0; j < 5; j++) {
      ...
      if (terminator(left, right, i) && this._stack.length > len) {
        if (!this._pending) this._maybeCollides(len, this._stack.length)
        else this._checks.push(this._stack.length, len)
      }
    }
...
function terminator (left, right, i) {
  if (left) return left.terminator(i)
  if (right) return right.terminator(i)
  return false
}

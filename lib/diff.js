  ...
  while (this._stack.length) {
    ...
    for (var j = 0; j < 5; j++) {
      ...
      if (leftSeq !== rightSeq) {
        ...
        if (!doneRight && rightSeq && notInBucket(j, rightSeq, leftBucket) && (!this._reconnect || this._db.feed.has(rightSeq))) {
          set(this._pushStack(rightLen++, i + 1), false, right)
        }
      }

      if (!doneLeft) {
        const pushLeft = !this._skipRightNull || (rightBucket[j] && (!this._reconnect || this._hasSeqInBucket(rightBucket, j)))
      ...
      if (!doneRight) {
        ...
        for (val = j; val < rightBucket.length; val += 5) {
          ...
          if (!seq) break
          if (seq !== leftSeq && notInBucket(j, seq, leftBucket) && (!this._reconnect || this._db.feed.has(seq))) {
...
Diff.prototype._hasSeqInBucket = function (bucket, val) {
  for (; val < bucket.length; val += 5) {
    if (bucket[val] && this._db.feed.has(bucket[val])) return true
  }
  return false
}
